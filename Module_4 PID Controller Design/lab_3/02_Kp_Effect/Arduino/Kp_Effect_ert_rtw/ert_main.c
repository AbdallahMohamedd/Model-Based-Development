...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Kp_Effect_M, 0);

  /* Set Final Simulation Time in Ticks */
  errorCode = extmodeSetFinalSimulationTime((extmodeSimulationTime_T) 130000);
  ...
  Kp_Effect_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(Kp_Effect_M->extModeInfo, (extmodeSimulationTime_T *)
    rteiGetPtrTFinalTicks(Kp_Effect_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Kp_Effect_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Kp_Effect_M);
